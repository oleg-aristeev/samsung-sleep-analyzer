...
- Сон фрагментированный/полифазный: много коротких сессий, время отхода ко сну плавает.
  Оценивай разрывность по sessions_count и longest_block_min в nights.csv (это сессии,
  размеченные самим трекером), а НЕ по awake-сегментам стадий — те включают микропробуждения.
- Пустые ячейки = датчик не дал данных за этот период.
"""
