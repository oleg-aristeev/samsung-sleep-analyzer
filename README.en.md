[🇷🇺 Русский](README.md) · 🇬🇧 English (this file)
...
Already deployed (free): **https://samsung-sleep-analyzer.streamlit.app** — just open it and
upload your `.zip`. Nothing is stored server-side.
...
In the **Samsung Health** app: **My page → ⋮ (top-right) → Settings → Download personal
data**. You'll receive a `samsunghealth_<user>_<id>.zip`. Point the tool at that `.zip`
(or at the unzipped folder).
